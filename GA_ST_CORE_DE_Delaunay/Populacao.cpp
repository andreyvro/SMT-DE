...
void Populacao::setIndividuo(unsigned int indice, Individuo individuo) {
	this->individuo.at(indice) = individuo;
}
...
const unsigned int Populacao::getDominante() {
	...
	#pragma omp parallel for reduction(min: melhorFitness) reduction(max: melhorInd)
...
const unsigned int Populacao::getPior() {
	...
	#pragma omp parallel for reduction(max: piorFitness, piorInd)
	for (unsigned int i = 1; i < this->qtdIndividuos; i++) {
		double fitness = this->individuo.at(i).getFitness();
...
const double Populacao::getDesvioPadrao() {
	...
	for (unsigned int i = 0; i < this->qtdIndividuos; i++) {
		variancia += pow(this->individuo.at(i).getFitness() - media, 2);
	}
