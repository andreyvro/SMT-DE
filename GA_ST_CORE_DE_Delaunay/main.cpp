...
int main(int argc, char* argv[]) {
    ...
    if (qtdInd < 3) {
        cout << "É necessário ao menos 3 indivíduos na população" << endl;
		exit(EXIT_FAILURE);
    }
// Inicio
...
// Arvore Mínima de Steiner
    cout << "Iniciando Algoritmo para SMT." << endl << endl;
    Populacao *pop = new Populacao;                      // População vazia
    GA ga(pop, qtdInd, indCruz, indMut, fitAtingir);     // Constrói população
...
// Log
    ...
    unsigned int melhorInd = pop->getDominante();
    pop->printIndividuo(melhorInd, exibirInfo, exibirInfo);     // Exibe dados da melhor árvore
    if (arqSaida != "") {
        string log = arqGrafo + "\n";
        ...
        log += pop->getLogIndividuo(melhorInd);                     // Fitness, Pontos Steiner, Cromossomo
